...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities):
    ...
    if available_sensors is None or len(available_sensors) == 0:
        _LOGGER.warning(f"Could not detect available sensors (obis-codes) using just 'import total' as default!")
...
class PPC_SMGWSensor(PPC_SMGWLocalEntity, SensorEntity):
    ...
    def state(self):
        ...
        reading = self.coordinator.ppc_smgw.get_reading_for_obis_code(self.entity_description.key)
        if reading is None:
            return None
...
class LastUpdatedSensor(PPC_SMGWLocalEntity, SensorEntity):
    ...
    def state(self):
        ...
        reading = self.coordinator.ppc_smgw._readings[0]
